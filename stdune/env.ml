module Var = struct
  module T = struct
    ...
    let compare =
      if Sys.win32 then fun a b -> String.compare (String.lowercase a) (String.lowercase b)
      else String.compare
  end
...
type t = { vars : string Map.t; mutable unix : string array option }
...
let add t ~var ~value = make (Map.add t.vars var value)
...
let diff x y =
  Map.merge x.vars y.vars ~f:(fun _k vx vy -> match vy with Some _ -> None | None -> vx) |> make

let update t ~var ~f = make (Map.update t.vars var ~f)

let of_string_map m = make (String.Map.foldi ~init:Map.empty ~f:(fun k v acc -> Map.add acc k v) m)
...
let cons_path t ~dir =
  make (Map.update t.vars "PATH" ~f:(fun _PATH -> Some (Bin.cons_path dir ~_PATH)))

let path env = match get env "PATH" with None -> [] | Some s -> Bin.parse_path s
