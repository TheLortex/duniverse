...
let rec filter_map l ~f =
  match l with
  | [] -> []
  | x :: l -> ( match f x with None -> filter_map l ~f | Some x -> x :: filter_map l ~f )

let rec filter_opt l =
  match l with
  | [] -> []
  | x :: l -> ( match x with None -> filter_opt l | Some x -> x :: filter_opt l )
...
type ('a, 'b) skip_or_either = Skip | Left of 'a | Right of 'b
...
let rec find l ~f = match l with [] -> None | x :: l -> if f x then Some x else find l ~f

let find_exn l ~f = match find l ~f with Some x -> x | None -> Exn.code_error "List.find_exn" []
...
let sort_uniq t ~compare = Dune_caml.List.sort_uniq (fun a b -> Ordering.to_int (compare a b)) t
...
let rec compare a b ~compare:f : Ordering.t =
  ...
  | [], _ :: _ -> Lt
  | _ :: _, [] -> Gt
  | x :: a, y :: b -> ( match (f x y : Ordering.t) with Eq -> compare a b ~compare:f | ne -> ne )

let rec assoc t x = match t with [] -> None | (k, v) :: t -> if x = k then Some v else assoc t x

let singleton x = [ x ]

let rec nth t i =
  match (t, i) with [], _ -> None | x :: _, 0 -> Some x | _ :: xs, i -> nth xs (i - 1)
