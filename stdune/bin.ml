...
let best_prog dir prog =
  ...
  if exists fn then Some fn
  else
    let fn = Path.relative dir (prog ^ exe) in
    if exists fn then Some fn else None

let which ~path prog =
  let rec search = function
    | [] -> None
    | dir :: rest -> ( match best_prog dir prog with None -> search rest | Some fn -> Some fn )
  in
  search path

let make ~path = match which ~path "gmake" with None -> which ~path "make" | some -> some
