...
module Make (R : Settings) () = struct
  ...
  module Table = struct
    type 'a t = { default_value : 'a; mutable data : 'a array }

    let create ~default_value = { default_value; data = Array.make R.initial_size default_value }

    let resize t =
      let n = new_size ~next:!next ~size:(Array.length t.data) R.resize_policy in
      let old_data = t.data in
      let new_data = Array.make n t.default_value in
      t.data <- new_data;
      Array.blit ~src:old_data ~src_pos:0 ~dst:new_data ~dst_pos:0 ~len:(Array.length old_data)

    let get t key = if key >= Array.length t.data then t.default_value else t.data.(key)
...
module No_interning (R : Settings) () = struct
  ...
  module Table = struct
    type 'a t = { default_value : 'a; data : (string, 'a) Hashtbl.t }

    let create ~default_value = { default_value; data = Hashtbl.create R.initial_size }

    let get t k = match Hashtbl.find t.data k with None -> t.default_value | Some s -> s

    let set t ~key ~data = Hashtbl.replace t.data ~key ~data
  end
end
