  ...
  module Make (Tag : Tag) = struct
    ...
    *)
    ...
    let embed_tag ~opening ~closing =
      ...
      Bytes.blit_string ~src:closing ~src_pos:0 ~dst:buf ~dst_pos:(2 + opening_len)
        ~len:closing_len;
    ...
    let setup ppf =
      ...
      pp_set_formatter_tag_functions ppf
        { funcs with mark_open_tag = extract_opening_tag; mark_close_tag = extract_closing_tag }
...
let concat ?(sep = Nop) = function [] -> Nop | [ x ] -> x | l -> Concat (sep, l)
