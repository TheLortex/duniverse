...
module Make (Key : Comparable.S) : S with type key = Key.t = struct
  ...
  include struct
    ...
    let find_opt key t = match M.find key t with x -> Some x | exception Not_found -> None
    ...
    let update ~key ~f t =
      match f (find_opt key t) with None -> M.remove key t | Some v -> M.add t ~key ~data:v
  ...
  let compare a b ~compare =
    M.compare a b ~cmp:(fun a b -> Ordering.to_int (compare a b)) |> Ordering.of_int
  ...
  let of_list =
    let rec loop acc = function
      | [] -> Result.Ok acc
      | (k, v) :: l -> (
        match find acc k with None -> loop (add acc k v) l | Some v_old -> Error (k, v_old, v) )
    in
    fun l -> loop empty l
  ...
  let of_list_map =
    let rec loop f acc = function
      ...
      | x :: l ->
          let k, v = f x in
          if not (mem acc k) then loop f (add acc k v) l else Error k
    in
    fun l ~f ->
      ...
      | Error k -> (
        match
          List.filter l ~f:(fun x -> match Key.compare (fst (f x)) k with Eq -> true | _ -> false)
        with
        | x :: y :: _ -> Error (k, x, y)
        | _ -> assert false )

  let of_list_map_exn t ~f =
    match of_list_map t ~f with Ok x -> x | Error _ -> Exn.code_error "Map.of_list_map_exn" []

  let of_list_exn l =
    match of_list l with Ok x -> x | Error _ -> Exn.code_error "Map.of_list_exn" []

  let of_list_reduce l ~f =
    List.fold_left l ~init:empty ~f:(fun acc (key, data) ->
        match find acc key with None -> add acc key data | Some x -> add acc key (f x data) )
  ...
  let of_list_reducei l ~f =
    List.fold_left l ~init:empty ~f:(fun acc (key, data) ->
        match find acc key with None -> add acc key data | Some x -> add acc key (f key x data) )

  let of_list_multi l =
    List.fold_left (List.rev l) ~init:empty ~f:(fun acc (key, data) -> add_multi acc key data)
  ...
  module Multi = struct
    ...
    let cons t k x = update t k ~f:(function None -> Some [ x ] | Some xs -> Some (x :: xs))

    let find t k = Option.value (find t k) ~default:[]
  end
end

let to_dyn to_list f g t = Dyn.Map (List.map ~f:(fun (k, v) -> (f k, g v)) (to_list t))
