...
module Make (Elt : Comparable.S) : S with type elt = Elt.t = struct
  ...
  include struct
    ...
    let map ~f t = M.elements t |> List.map ~f |> M.of_list
  ...
  let choose_exn t = match choose t with Some e -> e | None -> Exn.code_error "Set.choose_exn" []
end

let to_sexp to_list f t = Sexp.Encoder.list f (to_list t)
