...
let input_lines =
  let rec loop ic acc =
    match input_line ic with exception End_of_file -> List.rev acc | line -> loop ic (line :: acc)
  in
  fun ic -> loop ic []
...
struct
  ...
  let with_lexbuf_from_file fn ~f =
    with_file_in fn ~f:(fun ic ->
        ...
        lb.lex_curr_p <- { pos_fname = Path.to_string fn; pos_lnum = 1; pos_bol = 0; pos_cnum = 0 };
        f lb )
  ...
  let write_file ?binary fn data = with_file_out ?binary fn ~f:(fun oc -> output_string oc data)
  ...
  let read_file_and_normalize_eols fn =
    if not Sys.win32 then read_file fn
    else
