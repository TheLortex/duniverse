...
include struct
  ...
  let find_opt t key =
    match MoreLabels.Hashtbl.find t key with x -> Some x | exception Not_found -> None
...
module Make (H : Hashable.S) = struct
  ...
  include struct
    ...
    let find_opt t key = match find t key with x -> Some x | exception Not_found -> None
  end
  ...
  let of_list_exn l =
    ...
    | Result.Ok h -> h
    | Error (_, _, _) -> Exn.code_error "Hashtbl.of_list_exn duplicate keys" []
end
...
let to_dyn (type key) f g t =
  ...
  Map.to_dyn M.to_list f g (foldi t ~init:M.empty ~f:(fun key data acc -> M.add acc key data))

let to_sexp f g t = Dyn.to_sexp (to_dyn (Dyn.Encoder.via_sexp f) (Dyn.Encoder.via_sexp g) t)
