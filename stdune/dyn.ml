...
let rec pp = function
  ...
  | List x ->
      Pp.box
        [ Pp.char '['; Pp.concat_map ~sep:(Pp.seq (Pp.char ';') Pp.space) x ~f:pp; Pp.char ']' ]
  | Array a ->
      Pp.box
        [ Pp.verbatim "[|";
      ...
      Pp.box
        [ Pp.verbatim "map {";
          Pp.concat_map
            ...
            xs
            ~f:(fun (k, v) -> Pp.box [ pp k; Pp.space; Pp.verbatim ":"; Pp.space; pp v ]);
          Pp.verbatim "}"
        ]
  | Tuple x ->
      Pp.box
      ...
      Pp.vbox ~indent:2
        [ Pp.char '{';
          Pp.concat_map ~sep:(Pp.char ';') fields ~f:(fun (f, v) ->
              Pp.concat
                [ Pp.verbatim f; Pp.space; Pp.char '='; Pp.space; Pp.box ~indent:2 [ pp v ] ] );
          Pp.char '}'
        ]
  | Variant (v, []) -> Pp.verbatim v
  | Variant (v, xs) ->
      Pp.hvbox ~indent:2 [ Pp.verbatim v; Pp.space; Pp.concat_map ~sep:(Pp.char ',') xs ~f:pp ]
...
and pp_sexp = function
  ...
  | List l ->
      Pp.box ~indent:1
        [ Pp.char '('; Pp.hvbox [ Pp.concat_map l ~sep:Pp.space ~f:pp_sexp ]; Pp.char ')' ]
...
let rec to_sexp =
  ...
  | Record fields -> List.map fields ~f:(fun (field, f) -> (field, to_sexp f)) |> record
  | Variant (s, []) -> string s
...
module Encoder = struct
  ...
  let option f x = Option (Option.map ~f x)

  let via_sexp f x = Sexp (f x)
