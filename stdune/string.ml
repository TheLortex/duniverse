...
let rec check_suffix s ~suffix suffix_len offset i =
  i = suffix_len
  || (s.[offset + i] = suffix.[i] && check_suffix s ~suffix suffix_len offset (i + 1))
...
let lsplit2 s ~on =
  ...
  | i -> Some (sub s ~pos:0 ~len:i, sub s ~pos:(i + 1) ~len:(length s - i - 1))

let lsplit2_exn s ~on =
  match lsplit2 s ~on with
  | Some s -> s
  | None ->
      Exn.code_error "lsplit2_exn" [ ("s", Sexp.Encoder.string s); ("on", Sexp.Encoder.char on) ]
...
let index s ch = match index s ch with i -> Some i | exception Not_found -> None
...
let longest_prefix = function
  ...
  | x :: xs ->
      let rec loop len i =
        if i < len && List.for_all xs ~f:(fun s -> s.[i] = x.[i]) then loop len (i + 1) else i
      in
      let len = List.fold_left ~init:(length x) ~f:(fun acc x -> min acc (length x)) xs in
      sub ~pos:0 x ~len:(loop len 0)

let exists =
  let rec loop s i len f = if i = len then false else f (unsafe_get s i) || loop s (i + 1) len f in
...
let maybe_quoted s =
  ...
  if s == escaped || s = escaped then s else Printf.sprintf {|"%s"|} escaped
...
module Set = struct
  ...
  let pp fmt t =
    Format.fprintf fmt "Set (@[%a@])"
      (Format.pp_print_list Format.pp_print_string ~pp_sep:(fun fmt () -> Format.fprintf fmt "@ "))
      (to_list t)
...
module Map = struct
  ...
  let to_dyn f t =
    let open Dyn in
    Map (to_list t |> List.map ~f:(fun (k, v) -> (String k, f v)))
end
...
let findi =
  let rec loop s len ~f i =
    if i >= len then None else if f (String.unsafe_get s i) then Some i else loop s len ~f (i + 1)
  in
