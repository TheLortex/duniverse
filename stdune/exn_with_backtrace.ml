...
let try_with f = match f () with r -> Result.Ok r | exception exn -> Error (capture exn)
...
let to_sexp { exn; backtrace } =
  Sexp.List [ Atom (Printexc.to_string exn); Atom (Printexc.raw_backtrace_to_string backtrace) ]
