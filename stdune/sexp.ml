...
type t = Sexp0.t = Atom of string | List of t list
...
module Encoder = struct
  ...
  let record l = List (List.map l ~f:(fun (n, v) -> List [ Atom n; v ]))

  let unknown _ = Atom "<unknown>"

  let constr name = function [] -> Atom name | args -> List (Atom name :: args)
end
...
let rec to_dyn =
  let open Dyn0 in
  function
  | Atom s -> Variant ("Atom", [ String s ]) | List xs -> Variant ("List", List.map ~f:to_dyn xs)
