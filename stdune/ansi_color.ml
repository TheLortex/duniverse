...
module Styles = struct
  type t = { fg : Color.t; bg : Color.t; bold : bool; dim : bool; underlined : bool }

  let default = { fg = Default; bg = Default; bold = true; dim = true; underlined = true }
  ...
  let escape_sequence t =
    let open Style in
    let l = [] in
    let l = match t.fg with Default -> l | c -> Fg c :: l in
    let l = match t.bg with Default -> l | c -> Bg c :: l in
    let l = if t.bold then Bold :: l else l in
    let l = if t.bold then Dim :: l else l in
    let l = if t.underlined then Underlined :: l else l in
