let is_dir_sep =
  if Sys.win32 || Sys.cygwin then fun c -> c = '/' || c = '\\' || c = ':' else fun c -> c = '/'
...
let explode_path =
  ...
  fun path ->
    ...
    else match start [] path (String.length path - 1) with "." :: xs -> xs | xs -> xs
...
module External : sig
  ...
  let of_string t =
    if Filename.is_relative t then
      Exn.code_error "Path.External.of_string: relative path given" [ ("t", Sexp.Encoder.string t) ];
    make t

  let parse_string_exn ~loc t =
    if Filename.is_relative t then Exn.fatalf ~loc "path %s is not absolute" t;
    make t
...
*)
  ...
  let relative x y = match y with "." -> x | _ -> make (Filename.concat (to_string x) y)
  ...
  let parent t = if is_root t then None else Some (as_string t ~f:Filename.dirname)
  ...
  module Set = struct
    ...
    let of_listing ~dir ~filenames = of_list (List.map filenames ~f:(fun f -> relative dir f))
  end
...
module Relative : sig
  ...
  let basename t =
    if is_root t then Exn.code_error "Path.Local.basename called on the root" []
    else
      let t = to_string t in
      let len = String.length t in
    ...
    let relative_result t components =
      let rec loop t components =
        ...
        | ".." :: rest -> (
            ...
            else match parent t with None -> Error () | Some parent -> loop parent rest )
        | fn :: rest ->
            if is_root t then loop (make fn) rest else loop (make (to_string t ^ "/" ^ fn)) rest
      in
      loop t components
  ...
  let relative ?error_loc t path =
    if not (Filename.is_relative path) then
      Exn.code_error "Local.relative: received absolute path"
        [ ("t", to_sexp t); ("path", Sexp.Encoder.string path) ];
    match L.relative_result t (explode_path path) with
    | Result.Ok t -> t
    | Error () ->
        Exn.fatalf ?loc:error_loc "path outside the workspace: %s from %s" path (to_string t)
  ...
  let is_canonicalized =
    ...
    and before_dot_dot_slash s i =
      if i < 0 then false else match s.[i] with '/' -> false | _ -> in_component s (i - 1)
    and in_component s i =
      if i < 0 then true
      else match s.[i] with '/' -> before_slash s (i - 1) | _ -> in_component s (i - 1)
    in
    fun s ->
  ...
  let rec mkdir_p t =
    if is_root t then ()
      ...
      | Unix.Unix_error (ENOENT, _, _) as e ->
          ...
          if is_root parent then raise e
          else (
            mkdir_p parent;
            Unix.mkdir t_s 0o777 )
  ...
  let descendant t ~of_ =
    if is_root of_ then Some t
      ...
      if t_len > of_len && t.[of_len] = '/' && String.is_prefix t ~prefix:of_ then
  ...
  module Prefix = struct
    ...
    type t = { len : int; path : string; path_slash : string }

    let make p =
      if is_root p then Exn.code_error "Path.Local.Prefix.make" [ ("path", to_sexp p) ];
    ...
    let drop t p =
      ...
      else String.drop_prefix p ~prefix:t.path_slash |> Option.map ~f:make_path
  ...
  let explode p = if is_root p then [] else String.split (to_string p) ~on:'/'

  let to_string_maybe_quoted t = String.maybe_quoted (to_string t)

  let parent_exn t =
    match parent t with
    | None -> Exn.code_error "Path.Relative.parent:exn t is root" [ ("t", to_sexp t) ]
    | Some parent -> parent
  ...
  let set_root new_root =
    ...
    | Some root_dir ->
        Exn.code_error "set_root: cannot set root_dir more than once"
          [ ("root_dir", External.to_sexp root_dir); ("new_root_dir", External.to_sexp new_root) ]
  in
  let abs_root =
    lazy
      ( match !root_dir with
      | None -> Exn.code_error "root_dir: cannot use root dir before it's set" []
      | Some root_dir -> root_dir )
  in
  (abs_root, set_root)

module Kind = struct
  type t = External of External.t | Local of Local.t

  let to_absolute_filename t =
    match t with
    | External s -> External.to_string s
    | Local l -> External.to_string (External.relative (Lazy.force abs_root) (Local.to_string l))

  let to_string = function Local t -> Local.to_string t | External t -> External.to_string t

  let to_sexp t = Sexp.Encoder.string (to_string t)

  let of_string s =
    if Filename.is_relative s then Local (Local.of_string s) else External (External.of_string s)
  ...
  let mkdir_p = function Local t -> Local.mkdir_p t | External t -> External.mkdir_p t
...
module Build = struct
  ...
  let extract_build_context_dir t =
    ...
    | None -> Some (t, Source0.root)
    | Some (before, after) -> Some (Local.of_string before, after |> Source0.of_string)

  let extract_build_context_dir_exn t =
    match extract_build_context_dir t with
    | Some t -> t
    | None -> Exn.code_error "Path.Build.extract_build_context_dir_exn" [ ("t", to_sexp t) ]

  let extract_build_context_exn t =
    match extract_build_context t with
    | Some t -> t
    | None -> Exn.code_error "Path.Build.extract_build_context_exn" [ ("t", to_sexp t) ]
  ...
  let drop_build_context_exn t =
    ...
    | Some d -> d
    | None -> Exn.code_error "Path.Build.drop_build_context_exn" [ ("t", to_sexp t) ]

  let is_alias_stamp_file s = String.is_prefix (Local.to_string s) ~prefix:".aliases/"
    ...
    let set_build_dir (new_build_dir : Kind.t) =
              ...
              if Local.is_root p || Local.parent_exn p <> Local.root then
                Exn.fatalf
                  "@{<error>Error@}: Invalid build directory: %s\n\
                   The build directory must be an absolute path or a sub-directory of the root of \
                   the workspace."
      ...
      | Some build_dir ->
          Exn.code_error "set_build_dir: cannot set build_dir more than once"
            [ ("build_dir", Kind.to_sexp build_dir); ("new_build_dir", Kind.to_sexp new_build_dir) ]
    in
    let build_dir =
      lazy
        ( match !build_dir with
        | None -> Exn.code_error "build_dir: cannot use build dir before it's set" []
...
module T : sig
  ...
  type t = External of External.t | In_source_tree of Local.t | In_build_dir of Local.t
...
let is_root = function In_source_tree s -> Local.is_root s | In_build_dir _ | External _ -> false
...
let is_managed = function In_build_dir _ | In_source_tree _ -> true | External _ -> false
...
let parse_string_exn ~loc s =
  ...
  | s ->
      if Filename.is_relative s then make_local_path (Local.parse_string_exn ~loc s)
...
let external_of_local x ~root = External.to_string (External.relative root (Local.to_string x))
...
let append a b =
  ...
  | In_build_dir _ | External _ ->
      Exn.code_error "Path.append called with directory that's not in the source tree"
        [ ("a", to_sexp a); ("b", to_sexp b) ]
  | In_source_tree b -> append_relative a b

let basename t =
  match kind t with Local t -> Local.basename t | External t -> External.basename t
...
let parent_exn t =
  match parent t with
  | Some p -> p
  | None -> Exn.code_error "Path.parent:exn t is root" [ ("t", to_sexp t) ]

let is_strict_descendant_of_build_dir = function
  | In_build_dir p -> not (Local.is_root p)
  | In_source_tree _ | External _ -> false

let is_in_build_dir = function In_build_dir _ -> true | In_source_tree _ | External _ -> false

let is_in_source_tree = function In_source_tree _ -> true | In_build_dir _ | External _ -> false

let as_in_source_tree = function In_source_tree s -> Some s | In_build_dir _ | External _ -> None

let as_in_build_dir = function In_build_dir b -> Some b | In_source_tree _ | External _ -> None

let as_in_build_dir_exn t =
  match t with
  | External _ | In_source_tree _ ->
      Exn.code_error "[as_in_build_dir_exn] called on something not in build dir"
        [ ("t", to_sexp t) ]
  | In_build_dir p -> p
...
let extract_build_context_exn t =
  match extract_build_context t with
  | Some t -> t
  | None -> Exn.code_error "Path.extract_build_context_exn" [ ("t", to_sexp t) ]
...
let extract_build_context_dir_exn t =
  match extract_build_context_dir t with
  | Some t -> t
  | None -> Exn.code_error "Path.extract_build_context_dir_exn" [ ("t", to_sexp t) ]
...
let drop_optional_build_context t =
  match extract_build_context t with None -> t | Some (_, t) -> in_source_tree t
...
let drop_optional_build_context_src_exn t =
  ...
  | In_build_dir _ -> (
    ...
    | Some (_, s) -> s
    | None ->
        Exn.code_error "drop_optional_build_context_src_exn called on a build directory itself" []
    )
  | In_source_tree p -> p
...
let split_first_component t =
  ...
  | Local t, false -> (
      ...
      match String.lsplit2 t ~on:'/' with
      | None -> Some (t, root)
      | Some (before, after) -> Some (before, after |> Local.of_string |> in_source_tree) )
  | _, _ -> None
...
let explode_exn t =
  ...
  | Some s -> s
  | None -> Exn.code_error "Path.explode_exn" [ ("path", to_sexp t) ]
...
let readdir_unsorted =
  ...
  fun t ->
    try
      ...
      Exn.protect
        ~f:(fun () ->
          match loop dh [] with exception Unix.Unix_error (e, _, _) -> Error e | s -> Result.Ok s
          )
        ~finally:(fun () -> Unix.closedir dh)
    with Unix.Unix_error (e, _, _) -> Error e

let is_directory t = try Sys.is_directory (to_string t) with Sys_error _ -> false
...
let unlink_operation = if Sys.win32 then win32_unlink else Unix.unlink
...
let ensure_build_dir_exists () =
  ...
  | External p -> (
      ...
      | Unix.Unix_error (ENOENT, _, _) ->
          Exn.fatalf
            "Cannot create external build directory %s. Make sure that the parent dir %s exists." p
            (Filename.dirname p) )
...
let insert_after_build_dir_exn =
  let error a b =
    Exn.code_error "Path.insert_after_build_dir_exn"
      [ ("path", to_sexp a); ("insert", Sexp.Encoder.string b) ]
  in
  fun a b ->
    match a with
...
let rm_rf =
  let rec loop dir =
    Array.iter (Sys.readdir dir) ~f:(fun fn ->
        ...
        match Unix.lstat fn with { st_kind = S_DIR; _ } -> loop fn | _ -> unlink_operation fn );
    Unix.rmdir dir
  in
  fun t ->
    if not (is_managed t) then
      Exn.code_error "Path.rm_rf called on external dir" [ ("t", to_sexp t) ];
    let fn = to_string t in
    match Unix.lstat fn with exception Unix.Unix_error (ENOENT, _, _) -> () | _ -> loop fn

let mkdir_p = function
  | External s -> External.mkdir_p s
  | In_source_tree s -> Local.mkdir_p s
  | In_build_dir k -> Kind.mkdir_p (Kind.append_relative (Lazy.force Build.build_dir_kind) k)
...
let pp_in_source ppf t = pp ppf (drop_optional_build_context t)

let pp_debug ppf = function
  | In_source_tree s -> Format.fprintf ppf "(In_source_tree %S)" (Local.to_string s)
...
let is_suffix p ~suffix =
  match p with
  | In_build_dir l | In_source_tree l -> Local.is_suffix l ~suffix
...
module Source = struct
  ...
  let is_in_build_dir s = is_in_build_dir (of_local s)

  let to_local t = t
end

let set_of_source_paths set = Source.Set.to_list set |> List.map ~f:source |> Set.of_list
